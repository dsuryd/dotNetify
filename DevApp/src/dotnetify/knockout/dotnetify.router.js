...
dotnetify.ko.router.$inject = function(iVM) {
  ...
  iVM["$router"] = (function(iScope) {
    ...
    return {
      ...
      initRoot: function() {
        var vm = this;
        if (!vm.hasOwnProperty("RoutingState") || typeof vm.RoutingState.Root !== "function") return;
      ...
      initRouting: function() {
        var vm = this;
        if (!vm.hasOwnProperty("RoutingState") || typeof vm.RoutingState.Templates !== "function") return;

        var templates = vm.RoutingState.Templates();
        if (templates == null) return;

        // Initialize the router.
        if (!dotnetify.ko.router.$init) {
          dotnetify.ko.router.init();
          dotnetify.ko.router.$init = true;
        }
        ...
        $.each(templates, function(idx, template) {
          ...
          dotnetify.ko.router.mapTo(mapUrl, function(iParams) {
            dotnetify.ko.router.urlPath = "";

            // Construct the path from the template pattern and the params passed by PathJS.
            var path = template.UrlPattern();
            for (param in iParams) path = path.replace(":" + param, iParams[param]);
        ...
        if (dotnetify.ko.router.urlPath == "") dotnetify.ko.router.urlPath = activeUrl;
        vm.$router.routeUrl();
      }.bind(iScope),
        ...
        var callbackFn = function() {
          ...
          if (vm.hasOwnProperty("RoutingState") && typeof vm.RoutingState.Root === "function")
            $.each($(this).find("[data-vm]"), function(idx, element) {
              ...
              root =
                root != null
                  ? "/" + utils.trim(vm.RoutingState.Root()) + "/" + utils.trim(root)
      ...
      manualRouteTo: function(iPath, iTarget, iViewUrl, iJSModuleUrl) {
        ...
        template.ViewUrl = function() {
          return iViewUrl;
        };
        template.JSModuleUrl = function() {
          return iJSModuleUrl;
        };
      ...
      routeTo: function(iPath, iTemplate, iDisableEvent) {
        ...
        if (iDisableEvent != true && vm.hasOwnProperty("onRouteEnter"))
          if (vm.onRouteEnter(iPath, iTemplate) == false) return;

        // If target DOM element isn't found, redirect URL to the path.
        if ($("#" + iTemplate.Target()).length == 0) return dotnetify.ko.router.redirect(vm.$router.toUrl(iPath));
      ...
      routeUrl: function() {
        var vm = this;
        if (!vm.hasOwnProperty("RoutingState") || typeof vm.RoutingState.Templates !== "function") return false;

        var root = vm.RoutingState.Root();
        if (root == null) return false;

        // Get the URL path to route.
        var urlPath = dotnetify.ko.router.urlPath;
        ...
        if (utils.equal(urlPath, root)) {
          ...
          if (match.length > 0) vm.$router.routeTo("", match[0]);
          return;
        }
        ...
        if (utils.startsWith(urlPath, root)) {
          ...
          if (routeElem != null) {
            ...
            if (template != null) {
              ...
              if (utils.equal(dotnetify.ko.router.urlPath, vm.$router.toUrl(path))) dotnetify.ko.router.urlPath = "";

              // If route's not already active, route to it.
              if (!utils.equal(vm.RoutingState.Active(), path)) vm.$router.routeTo(path, template);

              return true;
            }
          } else if (dotnetify.ko.router.match(urlPath)) {
      ...
      toUrl: function(iPath) {
        ...
        return this.hasOwnProperty("RoutingState") && typeof (this.RoutingState.Root === "function")
          ? this.RoutingState.Root() + path
          : iPath;
      }.bind(iScope)
    };
  })(iVM);
...
ko.bindingHandlers.vmRoute = {
  update: function(element, valueAccessor, allBindings, viewModel, bindingContext) {
    ...
    var template = null;
    if (
      vm.hasOwnProperty("RoutingState") &&
      typeof vm.RoutingState.Templates === "function" &&
      vm.RoutingState.Templates() != null
    ) {
      var match = $.grep(vm.RoutingState.Templates(), function(iTemplate) {
    ...
    $(element)
      .attr("href", vm.$router.toUrl(path))
      .attr("data-vm-route", path)
      .click(function(iEvent) {
        iEvent.preventDefault();
        dotnetify.ko.router.pushState({}, "", $(this).attr("href"));
