		...
		return {
				...
				var callbackFn = function() {
					...
					if (vm.hasOwnProperty('RoutingState') && typeof vm.RoutingState.Root === 'function')
						$.each($(this).find('[data-vm]'), function(idx, element) {
							...
							root =
								root != null ? '/' + utils.trim(vm.RoutingState.Root()) + '/' + utils.trim(root) : vm.RoutingState.Root();
			...
			routeTo: function(iPath, iTemplate, iDisableEvent) {
				...
				if (iTemplate.Target() === null) {
					console.error(
						"router> the Target for template '" + iTemplate.Id() + "' was not set.  Use vm.onRouteEnter() to set the target."
					);
					return;
				}
