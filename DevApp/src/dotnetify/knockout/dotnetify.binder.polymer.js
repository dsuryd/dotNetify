  ...
  dotnetify.binder.extensions["polymer-paper"] = {
    ...
    setBindings: function (elem, bind) {
      ...
      var renameTag = function (tag, newTagName) {
        ...
        html = html.replace(
          new RegExp(tag[0].tagName.toLowerCase(), "g"),
          newTagName
        );
      ...
      if (
        ...
        if (
          listbox.length > 0 &&
          vm.hasOwnProperty(propOptions) &&
          vm.hasOwnProperty(propValue)
        ) {
          ...
          vm[id].$selected = function (value) {
            ...
            if (typeof value === "undefined") {
              var match = ko.utils.arrayFirst(vm[propOptions](), function (i) {
                return typeof i[key] === "function" && i[key]() == vm[id]();
              });
          ...
          if (typeof bindListbox === "undefined") bindListbox = "";
          ...
          if (item.length > 0 && vm.hasOwnProperty(propText)) {
            ...
            if (typeof bindItem === "undefined") bindItem = "";
        ...
        $(elem).on("paper-radio-group-changed", function () {
          vm[id](this.selected);
        });
        vm[id].$bound = true;
      } else if (tagName == "paper-slider") {
        bind += "attr: { value: " + id + " }";
        $(elem).on("value-change", function () {
          vm[id](this.value);
        });
        vm[id].$bound = true;
      } else if (tagName == "paper-tabs") {
        bind += "attr: { selected: " + id + " }";
        $(elem).on("iron-select", function () {
          vm[id](this.selected);
        });
        vm[id].$bound = true;
    ...
    postBindings: function (elem, bind) {
      ...
      if (tagName == "paper-dropdown-menu") {
        ...
        $.each($(elem).find("ko-paper-listbox"), function () {
          ...
          newListbox.on("iron-select", function () {
            vm[id].$selected(this.selected);
          });
        });
      }
    }
