        ...
        return {
          setBindings: function (elem, bind) {
            ...
            if (elem.tagName.toLowerCase() == "select") {
              ...
              var koOptionsNotations = [
                "optionsCaption",
                "optionsText",
                "optionsValue"
              ];
              for (i in koOptionsNotations)
                bind = koBinder.bindToObservableValue.apply(vm, [
                  koOptionsNotations[i],
                  elem,
                  bind
                ]);
            } else {
              ...
              for (i in notations) {
                ...
                if (vm.hasOwnProperty(prop)) vm[prop].$bound = true;
              }
            }
          ...
          setIdBinding: function (elem, bind) {
            ...
            if (!vm.$binder.bindable(id)) return bind;
            ...
            if (tagName == "input") {
              ...
              if (type == "search") bind += "textInput: " + id;
              else if (type == "checkbox" || type == "radio")
                bind += "checked: " + id;
              else if (type == "button") bind += "vmCommand: " + id;
              else bind += "value: " + id;
            } else if (tagName == "button") bind += "vmCommand: " + id;
            else if (tagName == "a") bind += "vmRoute: " + id;
            else if (tagName == "select" || tagName == "textarea")
              bind += "value: " + id;
            else if (vm.hasOwnProperty(id) && "push" in vm[id])
              // If property is an observable array.
              bind += "foreach: " + id;
            else if (tagName.indexOf("-") < 0)
              // If element isn't a web component.
              bind += "html: " + id;
          ...
          setAttrBindings: function (elem, bind) {
            ...
            var startsWith = function (iStr, iValue) {
              return (
                iStr.toLowerCase().slice(0, iValue.length) ==
                iValue.toLowerCase()
              );
            };
            ...
            for (prop in vm) {
              if (
                prop != id &&
                startsWith(prop, id + "_") &&
                vm.$binder.bindable(prop) &&
                typeof vm[prop].$bound === "undefined"
              )
                props[prop] = vm[prop];
            }

            // If no property to bind, leave now.
            if (Object.keys(props).length === 0) return bind;
            ...
            for (prop in props) {
              ...
              var nth = 0;
              var attr = prop.replace(/_/g, function (match, i) {
                return ++nth === 2 ? "-" : match;
              });

              var match = /.*_(.*)/.exec(attr);
              if (match != null) {
                if (attr != prop) match[1] = "'" + match[1] + "'";
        ...
        init: function () {
          ...
          $.each(vm.$element.find("[id]"), function (idx, elem) {
            ...
            for (ext in dotnetify.binder.extensions)
              bind = dotnetify.binder.extensions[ext].setBindings.apply(vm, [
                elem,
                bind
              ]);
          ...
          if (prop == "$data") return true;
            ...
            if (parent instanceof Array)
              return parent.length > 0
                ? parent[0].hasOwnProperty(prop) && ko.observable(prop)
                : true;
          }
