...
export default class DotnetifyVMRouter implements IDotnetifyVMRouter {
  ...
  constructor(vm: DotnetifyVM, router: DotnetifyRouter) {
    this.vm = vm;
    this.router = router;
  ...
  dispatchActiveRoutingState(iPath: string) {
  ...
  manualRouteTo(iPath: string, iTarget: string, iViewUrl?: any, iJSModuleUrl?: string) {
    const vm = this.vm;
    const template: RoutingTemplateType = { Id: "manual", UrlPattern: null, Target: iTarget, ViewUrl: iViewUrl, JSModuleUrl: iJSModuleUrl };
  ...
  route(iRoute: RouteType, iTarget: string) {
  ...
  routeUrl(redirectUrlPath?: string): boolean {
